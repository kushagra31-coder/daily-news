...
export interface IArticleRepository {
  ...
  getLiveArticlesCount(category?: string): Promise<number>;
...
export class ArticleRepository implements IArticleRepository {
  ...
  async getLiveArticlesCount(category?: string): Promise<number> {
    ...
    const res = await this.db.select({ count: sql`count(*)` })
      .from(articles)
  ...
  async getStats(): Promise<any> {
    ...
    const verified = total.filter((a: any) => a.verification_score > 0.5);
    const expiring = total.filter((a: any) => a.expires_at > now && a.expires_at < oneHourFromNow);
